...
my_hits_z_1 <-my_hits_z[complete.cases(my_hits_z), ]
cat <- rep("Tissue Specificity - Tau", length(my_hits_z_1$Orthogroup))
my_hits_z_1 <- cbind(my_hits_z_1,cat)
...
my_hits_z <- my_hits %>%
  left_join(final, by = c("Orthogroup"))
my_hits_z_2 <-my_hits_z[complete.cases(my_hits_z), ]
cat <- rep("Node Betweenness", length(my_hits_z_2$Orthogroup))
my_hits_z_2 <- cbind(my_hits_z_2,cat)
...
# Plot
df <- rbind(df1,df2)
my_hits_z <- rbind(my_hits_z_1[,21:22],my_hits_z_2[20:21])
...
ggplot(df, aes(cat, mean(final_to_plot))) +        # ggplot2 plot with confidence intervals
    ...
    geom_point(aes(y=mean(my_hits_z$z),x = cat),colour="red", shape = 17, size = 4) +
