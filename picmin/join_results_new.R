...
ortho_counts <- table(df$Orthogroup)
species_counts <- table(df$species)
valid_orthogroups <- names(ortho_counts)[ortho_counts >= 7]
df <- df[df$Orthogroup %in% valid_orthogroups, ]


# Assuming your dataframe is named 'df'
df <- df[, c("Orthogroup", colnames(df)[!colnames(df) %in% "Orthogroup"])]
df$ortho_DS <- ave(df$ortho_DS, df$species, FUN = assign.pvalues)
saveRDS(df, "orthogroup_results.rds")
